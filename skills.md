...
TOD ships five agent-agnostic skills under [`skills/`](skills/). Each is a
...
| Skill | Purpose |
|-------|---------|
| [`using-tod`](skills/using-tod/SKILL.md) | Umbrella guide that covers any OneDev interaction via `tod`. |
| [`edit-build-spec`](skills/edit-build-spec/SKILL.md) | Create or update `.onedev-buildspec.yml` using `tod build get-spec-schema` and `tod build check-spec`. |
| [`investigate-build-problems`](skills/investigate-build-problems/SKILL.md) | Debug a failing build via `tod build get`, `tod build get-log`, `tod build get-file-content`, and `tod build get-changes-since-success`. |
| [`review-pull-request`](skills/review-pull-request/SKILL.md) | Review a pull request via `tod pr get`, `tod pr get-patch`, `tod pr get-file-content`, and `tod pr approve` / `tod pr request-changes` / `tod pr add-comment`. |
| [`generate-commit-message`](skills/generate-commit-message/SKILL.md) | Compose a commit message that respects `tod get-commit-message-requirement`, derives the subject from the active issue (`tod issue current-reference`/`tod issue get`) when set, and adds a meaningful body or footer. |
...
`access-token`. Run `tod config set` to create one. See the main
