...
// apiPostBytes performs an authenticated POST against a tod endpoint with no
...
// the same key is provided more than once the values are collected into a
// JSON array.
func collectFieldFlags(fields []string) (map[string]interface{}, error) {
	...
	for _, entry := range fields {
		...
		val := parseFieldValue(entry[idx+1:])
